...
const QuickTrade = () => {
  ...
  return (
    <div className="md:col-span-6 xl:col-span-3 w-full bg-gradient-to-tl from-darkOne to-darkTwo p-5 rounded">
      <div className="flex items-center justify-between">
        ...
        <div className="relative w-60 h-10 flex items-center bg-darkBG rounded-full">
          <div
            className={`w-1/2 h-full rounded-full bg-primary  transition-transform ${isBuyOpen ? "translate-x-0" : "translate-x-full"
              } duration-200 ease-in-out`}
          ></div>
          <button
            onClick={() => setIsBuyOpen(true)}
            className={`absolute w-1/2 h-full bg-transparent transition-all ${isBuyOpen ? "text-white" : "text-gray-300"
          ...
          <button
            ...
            className={`absolute w-1/2 transform translate-x-full h-full bg-transparent transition-all ${!isBuyOpen ? "text-white" : "text-gray-300"
      ...
      {/* content */}
      {isBuyOpen ? <QuickBuy cryptoCurrency={cryptoCurrency} cryptoRefetch={cryptoRefetch} flatCurrency={flatCurrency} flatRefetch={flatRefetch} /> : <QuickSell />}
    </div>
  );
};
