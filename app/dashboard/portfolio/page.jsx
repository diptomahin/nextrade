...
// data fetch in all coin in buying  market page 
  ...
  useEffect(()=>{
    if(purchasedAssets.length >0){
      setCryptoData(purchasedAssets.filter(data=>data.type === "crypto coin"))
      setCurrencyData(purchasedAssets.filter(data=>data.type === "flat coin"))
    }
  },[purchasedAssets])

// data information create object 
  const createCryptoData = (assetName,assetKey,assetImg,assetType,assetBuyingPrice,currentPrice,assetPortion,totalInvestment,assetBuyerUID,assetBuyerEmail)=>{
    return {assetName,assetKey,assetImg,assetBuyingPrice,currentPrice,assetPortion,totalInvestment,assetBuyerUID,assetBuyerEmail}
  }
